...
?>
	...
	<form action="/admin/prompts/action" method="post" novalidate enctype="multipart/form-data" id="form">
					...
					<div class="form-floating mb-3">
						<select name="API_MODEL" class="form-control form-select" id="floatingAPIModel" required>
							<optgroup label="GPT-4">
								<option value="gpt-4-1106-preview" <?php if (isset($edit) && $edit && ($get->API_MODEL ?? '') == 'gpt-4-1106-preview') { echo 'selected'; } ?>>gpt-4-1106-preview (GPT4 turbo)</option>
		...
		<fieldset class="border rounded-2 p-3 mb-4">
			<legend><h5> Show Vision (Image reading) in chat</h5></legend>
			<div class="row align-middle">


				<div class="col-md-12">
					<div class="form-check form-switch mb-3 custom-switch">
						<input class="form-check-input" type="checkbox" id="floatingDisplayVision" <?php if (isset($edit) && $edit && ($get->use_vision ?? 0) == 1) { echo 'checked'; } ?> onchange="updateSwitchValue('floatingDisplayVision', 'hiddenDisplayVision')">
						<input type="hidden" name="use_vision" id="hiddenDisplayVision" value="<?php echo isset($edit) && $edit ? $get->use_vision : 0; ?>">
						<label class="form-check-label" for="floatingDisplayVision">Use Vision in chat</label>
					</div>
				</div>
		...
		<fieldset class="border rounded-2 p-3 mb-4">
			<legend><h5> Show microphone in chat</h5></legend>
			<div class="row align-middle">
				...
				<div class="col-md-3">
					<div class="form-check form-switch mb-3 custom-switch">
						<input class="form-check-input" type="checkbox" id="floatingDisplayAvatar" <?php if ((isset($edit) && $edit && ($get->use_mic_whisper ?? 0) == 1) || (!isset($edit) || !$edit)) { echo 'checked'; } ?> onchange="updateSwitchValue('floatingDisplayAvatar', 'hiddenDisplayAvatar')">
						<input type="hidden" name="use_mic_whisper" id="hiddenDisplayAvatar" value="<?php echo isset($edit) && $edit ? $get->use_mic_whisper : 1; ?>">
						<label class="form-check-label" for="floatingDisplayAvatar">Use Whisper on the microphone</label>
						<i class="bi bi-question-circle" tabindex="-1" data-bs-toggle="tooltip" title="Whisper is an OpenAI automatic speech recognition (ASR) system trained on 680,000 hours of multilingual and multitask supervised data collected from the web. With Whisper, the user's speech from the microphone becomes more easily interpreted, regardless of accent, background noise, and technical language."></i>
					</div>
				</div>
